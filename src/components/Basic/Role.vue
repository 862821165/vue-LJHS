  ...
  <Layout :style="{padding: '0 24px 24px'}">
    ...
    <Content>
      ...
      <Row :style="{margin:'10px 0'}">
        <i-col span="18">
          <ButtonGroup>
            <Button icon="md-trash" type="error" @click="remove_modal = true">删除</Button>
          </ButtonGroup>
          <Button icon="md-person-add" type="success" @click="modal_add = true">添加</Button>
        </i-col>
        <i-col span="6">
          <i-input v-model="search_value" placeholder="角色名称">
    ...
    <Drawer
      ...
      <Form ref="formItem" :model="formItem" :rules="ruleValidate" :label-width="70">
        <FormItem label="角色名称" prop="name">
          <Input v-model.trim="formItem.name" placeholder="输入角色名"></Input>
        </FormItem>
    ...
    <Drawer
      ...
      <Form ref="formItem_edit" :model="formItem_edit" :rules="ruleValidate_edit" :label-width="70">
        <FormItem label="角色名称" prop="name">
          <Input v-model.trim="formItem_edit.name" placeholder="输入角色名"></Input>
        </FormItem>
        <FormItem label="角色备注" prop="description">
            ...
            render: (h, params) => {
              return h('div', [
                h('Button', {
                  ...
                  on: {
                    click: () => {
                      this.RPermissions_href(params.row)
                    }
                  }
                }, '权限设置'),
                h('Button', {
    ...
    methods: {
      ...
      remove_data() {
        ...
        *=====================================================*/
        if (this.selectd_ids.length > 0) {
          ...
          this.$http({
            method: 'post',
            url: this.CD_.qj_url + 'index/Role/removeRole',
            data: {
            ...
            if (json.status == 1) {
              ...
              this.$Notice.success({title: '成功删除角色~~'});
              this.getRole_data();
            } else {
              this.$Notice.error({title: '删除角色失败~~'});
            }

          }.bind(this)).catch(function (err) {
            this.$Notice.error({title: '删除角色失败，错误：' + err});
          }.bind(this))
        } else {
          this.$Notice.warning({
            title: '请勾选要删除的角色~~',
        ...
        *=====================================================*/
        ...
        this.$refs[data].validate((valid) => {
          if (valid) {
            ...
            this.$http({
              method: 'post',
              url: this.CD_.qj_url + 'index/Role/addRole',
              data: {
              ...
              var json = res.data
              if (json.status) {
                this.$Notice.success({title: '成功编辑角色~~~'});
                this.getRole_data();
                this.$Loading.finish();
                this.modal_edit = false ;
              } else {
                this.$Notice.error({title: '编辑角色失败~~~'});
                this.$Loading.error();
              }
              this.isLoading = false;
            }.bind(this)).catch(function (err) {
              this.$Notice.error({title: '编辑角色失败，错误：' + err});
              this.$Loading.error();
              this.isLoading = false;
            }.bind(this))
          } else {
            this.$Notice.error({title: '角色信息填写有误'});
            this.isLoading = false;
          }
        })
      },
        ...
        this.$refs[data].validate((valid) => {
          if (valid) {
            ...
            this.$http({
              ...
              var json = res.data
              if (json.status) {
                this.$Notice.success({title: '成功添加角色~~~'});
                this.getRole_data();
                this.$Loading.finish();
                this.modal_add = false ;
              } else {
                this.$Notice.error({title: '添加角色失败~~~'});
                this.$Loading.error();
              }
              this.isLoading = false;
            }.bind(this)).catch(function (err) {
              this.$Notice.error({title: '添加角色失败',desc:'错误：'+ err});
              this.$Loading.error();
              this.isLoading = false;
            }.bind(this))
          } else {
      ...
      getRole_data() {
        ...
        *=====================================================*/
        this.$http({
          method: 'post',
          url: this.CD_.qj_url + 'index/Role/getRole',
          data: {
          ...
          var json = res.data;
          if (json.status) {
            this.table_data = json.role_data;
            this.page = json.page;
            this.$Loading.finish();
          } else {
            this.$Notice.error({title: '角色不存在~~~'});
            this.$Loading.error();
          }
