<template>
  <Layout :style="{padding: '0 24px 24px'}">
    ...
    <Card :bordered="false">
      <Select v-model="model_dp" @on-change="change_select" clearable filterable style="width:300px" placeholder="请先选择需要设置权限的部门">
    ...
    <div class="demo-spin-article"  style="position: relative;margin-top: 15px">
      ...
      <Spin fix  v-if="spinShow">请先选择需要设置权限的部门</Spin>
    </div>
...
//                this.$Notice.error({title: '请选择可以访问的菜单'});
      ...
      seve_menu(){
        ...
        *=====================================================*/
        ...
        this.$http({
          method: 'post',
          url: this.CD_.qj_url+'index/Menu/seve_department',
          data:{
      ...
      getSelsctRole(){
        ...
        this.$http({
          method: 'get',
          url: this.CD_.qj_url+'index/Department/getSelsctDp'
        }).then(function(res){
          var json = res.data ;
          if(json.status){
            ...
            var _this = this ;
            setTimeout(function () {
              _this.model_dp = _this.$route.query.model_dp ;
              if(_this.model_dp !== undefined){
